...
> (administrador de um estacionamento) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar as vagas**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista vagas em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar os tickets de entrada/saída, e cada carro gera um ticket."*
...
- 📨 **Cliente (falando demais):** *"o vaga torce pro tal time, faz aniversário em maio, indicou um amigo…"* — 90% irrelevante.
...
- 📨 **Cliente:** *"Quero que cada ticket seja uma 'coisa' de verdade no sistema, não uma linha solta."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode ocupar uma vaga que já está ocupada."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode ocupar uma vaga que já está ocupada" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: o valor da hora ficou negativo. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode ocupar uma vaga que já está ocupada": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: uma vaga recebe um veículo por vez; cada ticket registra entrada e saída — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (uma vaga recebe um veículo por vez; cada ticket registra entrada e saída) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: mensalista (vs avulso)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **mensalista (vs avulso)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse mensalista (vs avulso) **se comporta diferente**: tem vaga garantida e não paga por hora."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **mensalista (vs avulso)** faça o seu (tem vaga garantida e não paga por hora) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de cobrança (por hora, diária, mensal) sem refazer o sistema."*
- 💡 **Exercita:** classe abstrata + **interface** = contrato plugável (baixo acoplamento).
- 📦 **Entregável:** uma **interface** `Tarifador` com **2 implementações** intercambiáveis.
- 🎯 **Direcionamento:** criem a interface `Tarifador` para a forma de cobrança (por hora, diária, mensal) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece registrar a entrada e a saída de um veículo."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "registrar a entrada e a saída de um veículo" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📨 **Cliente:** *"A vaga passa por **fases** e **não pode pular etapa**."*
- 💡 **Exercita:** estados, transições, **guardas** (regra de negócio na transição).
- 📦 **Entregável:** **máquina de estados** de a vaga: `livre → ocupada → livre`.
...
- 📨 **Cliente:** *"Escreve o **processo da entrada do veículo ao pagamento na saída**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da entrada do veículo ao pagamento na saída com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Ticket calcula o valor, libera a cancela E emite o recibo — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Ticket calcula o valor, libera a cancela E emite o recibo — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: o cálculo de tarifa por fração de hora confuso — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem o cálculo de tarifa por fração de hora confuso e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de controle de vagas e cobrança. Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (controle de vagas e cobrança) e reorganizem o código em `com.estacionamento.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em as cancelas de entrada/saída e um servidor central."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (as cancelas de entrada/saída e um servidor central) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: registrar a entrada e a saída de um veículo, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "registrar a entrada e a saída de um veículo" do início ao fim e apliquem **um** padrão onde couber (não force).
