...
> (coordenador de uma escola) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os alunos**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista alunos em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar as matrículas dos alunos nas turmas, e um aluno cursa várias turmas."*
...
- 📨 **Cliente (falando demais):** *"o aluno torce pro tal time, faz aniversário em maio, indicou um amigo…"* — 90% irrelevante.
...
- 📨 **Cliente:** *"Quero que cada matrícula seja uma 'coisa' de verdade no sistema, não uma linha solta."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode matricular o aluno numa turma sem vaga."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode matricular o aluno numa turma sem vaga" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: lançaram nota 15 numa escala de 0 a 10. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode matricular o aluno numa turma sem vaga": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: uma turma tem muitos alunos; um aluno está em muitas turmas (a matrícula liga os dois) — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (uma turma tem muitos alunos; um aluno está em muitas turmas (a matrícula liga os dois)) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: aluno bolsista (vs pagante)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **aluno bolsista (vs pagante)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse aluno bolsista (vs pagante) **se comporta diferente**: tem mensalidade reduzida ou isenta."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **aluno bolsista (vs pagante)** faça o seu (tem mensalidade reduzida ou isenta) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** o critério de cálculo da média (por prova, por trabalho…) sem refazer o sistema."*
- 💡 **Exercita:** classe abstrata + **interface** = contrato plugável (baixo acoplamento).
- 📦 **Entregável:** uma **interface** `CriterioDeAvaliacao` com **2 implementações** intercambiáveis.
- 🎯 **Direcionamento:** criem a interface `CriterioDeAvaliacao` para o critério de cálculo da média (por prova, por trabalho…) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece matricular um aluno numa turma."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "matricular um aluno numa turma" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📨 **Cliente:** *"A matrícula passa por **fases** e **não pode pular etapa**."*
- 💡 **Exercita:** estados, transições, **guardas** (regra de negócio na transição).
- 📦 **Entregável:** **máquina de estados** de a matrícula: `ativa → trancada → ativa; ou cancelada`.
...
- 📨 **Cliente:** *"Escreve o **processo da matrícula ao fechamento das notas do bimestre**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da matrícula ao fechamento das notas do bimestre com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Aluno calcula a média, monta o boletim E envia ao responsável — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Aluno calcula a média, monta o boletim E envia ao responsável — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: o cálculo de média/aprovação com ifs aninhados — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem o cálculo de média/aprovação com ifs aninhados e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de secretaria (matrícula) e notas/boletim. Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (secretaria (matrícula) e notas/boletim) e reorganizem o código em `com.escola.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em computadores na secretaria e um portal para os pais verem as notas."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (computadores na secretaria e um portal para os pais verem as notas) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: matricular um aluno numa turma, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "matricular um aluno numa turma" do início ao fim e apliquem **um** padrão onde couber (não force).
