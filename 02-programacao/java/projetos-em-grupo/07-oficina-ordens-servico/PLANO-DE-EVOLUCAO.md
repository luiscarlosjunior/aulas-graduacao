...
> (dono de uma oficina mecânica) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os veículos dos clientes**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista veículos em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar as ordens de serviço, e cada OS lista vários serviços e peças."*
...
- 📨 **Cliente (falando demais):** *"o veículo torce pro tal time, faz aniversário em maio, indicou um amigo…"* — 90% irrelevante.
...
- 📨 **Cliente:** *"Quero que cada ordem de serviço seja uma 'coisa' de verdade no sistema, não uma linha solta."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode executar a OS antes de o orçamento ser aprovado."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode executar a OS antes de o orçamento ser aprovado" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: o valor da mão de obra ficou negativo. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode executar a OS antes de o orçamento ser aprovado": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: uma OS é de um veículo e é feita de vários itens (serviços e peças) — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (uma OS é de um veículo e é feita de vários itens (serviços e peças)) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: cliente com contrato de frota (vs avulso)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **cliente com contrato de frota (vs avulso)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse cliente com contrato de frota (vs avulso) **se comporta diferente**: tem tabela de preço própria e prazo diferenciado."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **cliente com contrato de frota (vs avulso)** faça o seu (tem tabela de preço própria e prazo diferenciado) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de pagamento (à vista, faturado) sem refazer o sistema."*
...
- 🎯 **Direcionamento:** criem a interface `MeioDePagamento` para a forma de pagamento (à vista, faturado) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece abrir uma OS e fechar o serviço."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "abrir uma OS e fechar o serviço" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📨 **Cliente:** *"A ordem de serviço passa por **fases** e **não pode pular etapa**."*
- 💡 **Exercita:** estados, transições, **guardas** (regra de negócio na transição).
- 📦 **Entregável:** **máquina de estados** de a ordem de serviço: `aberta → orçada → aprovada → em execução → concluída → paga`.
...
- 📨 **Cliente:** *"Escreve o **processo da entrada do veículo à entrega**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da entrada do veículo à entrega com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe OrdemServico soma o valor, emite a nota E avisa o cliente — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe OrdemServico soma o valor, emite a nota E avisa o cliente — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: o cálculo do total (mão de obra + peças + desconto) confuso — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem o cálculo do total (mão de obra + peças + desconto) confuso e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de orçamento/execução e faturamento. Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (orçamento/execução e faturamento) e reorganizem o código em `com.oficina.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em computadores na recepção e na oficina, com banco central."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (computadores na recepção e na oficina, com banco central) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: abrir uma OS e fechar o serviço, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "abrir uma OS e fechar o serviço" do início ao fim e apliquem **um** padrão onde couber (não force).
