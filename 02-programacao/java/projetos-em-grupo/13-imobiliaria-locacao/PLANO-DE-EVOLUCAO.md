...
> (dono de uma imobiliária) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os imóveis**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista imóveis em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar os contratos de aluguel, e um proprietário pode ter vários imóveis."*
...
- 📨 **Cliente (falando demais):** *"o imóvel torce pro tal time, faz aniversário em maio, indicou um amigo…"* — 90% irrelevante.
...
- 📨 **Cliente:** *"Quero que cada contrato seja uma 'coisa' de verdade no sistema, não uma linha solta."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode alugar um imóvel que já está com contrato ativo."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode alugar um imóvel que já está com contrato ativo" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: o valor do aluguel ficou negativo. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode alugar um imóvel que já está com contrato ativo": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: um proprietário tem vários imóveis; cada contrato liga um imóvel a um inquilino — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (um proprietário tem vários imóveis; cada contrato liga um imóvel a um inquilino) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: imóvel comercial (vs residencial)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **imóvel comercial (vs residencial)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse imóvel comercial (vs residencial) **se comporta diferente**: tem regras de reajuste e impostos diferentes."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **imóvel comercial (vs residencial)** faça o seu (tem regras de reajuste e impostos diferentes) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de cobrança do aluguel (boleto, débito, Pix) sem refazer o sistema."*
...
- 🎯 **Direcionamento:** criem a interface `MeioDeCobranca` para a forma de cobrança do aluguel (boleto, débito, Pix) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece fechar um contrato de locação."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "fechar um contrato de locação" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📨 **Cliente:** *"O imóvel passa por **fases** e **não pode pular etapa**."*
- 💡 **Exercita:** estados, transições, **guardas** (regra de negócio na transição).
- 📦 **Entregável:** **máquina de estados** de o imóvel: `disponível → alugado → disponível; ou em manutenção`.
...
- 📨 **Cliente:** *"Escreve o **processo da visita ao fechamento do contrato**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da visita ao fechamento do contrato com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Contrato calcula o reajuste, gera o boleto E arquiva o PDF — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Contrato calcula o reajuste, gera o boleto E arquiva o PDF — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: o cálculo de reajuste anual com muitos índices — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem o cálculo de reajuste anual com muitos índices e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de cobrança e contratos. Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (cobrança e contratos) e reorganizem o código em `com.imobiliaria.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em um site de busca de imóveis, um servidor e um banco."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (um site de busca de imóveis, um servidor e um banco) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: fechar um contrato de locação, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "fechar um contrato de locação" do início ao fim e apliquem **um** padrão onde couber (não force).
