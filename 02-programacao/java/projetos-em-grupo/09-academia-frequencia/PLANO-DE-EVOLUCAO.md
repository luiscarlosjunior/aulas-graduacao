...
> (dono de uma academia) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Ah, também preciso registrar os check-ins de treino, e um aluno acumula vários por mês."*
...
- 📨 **Cliente:** *"Quero que cada check-in seja uma 'coisa' de verdade no sistema, não uma linha solta."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode fazer check-in com o plano vencido."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode fazer check-in com o plano vencido" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: o valor do plano ficou negativo. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode fazer check-in com o plano vencido": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: um aluno tem um plano ativo e muitos check-ins — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (um aluno tem um plano ativo e muitos check-ins) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: aluno com plano família (vs individual)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **aluno com plano família (vs individual)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse aluno com plano família (vs individual) **se comporta diferente**: divide a mensalidade entre dependentes."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **aluno com plano família (vs individual)** faça o seu (divide a mensalidade entre dependentes) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de pagamento da mensalidade (cartão recorrente, Pix) sem refazer o sistema."*
...
- 🎯 **Direcionamento:** criem a interface `MeioDePagamento` para a forma de pagamento da mensalidade (cartão recorrente, Pix) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece fazer check-in na catraca."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "fazer check-in na catraca" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📨 **Cliente:** *"O plano passa por **fases** e **não pode pular etapa**."*
- 💡 **Exercita:** estados, transições, **guardas** (regra de negócio na transição).
- 📦 **Entregável:** **máquina de estados** de o plano: `ativo → vencido → ativo (após pagamento); ou cancelado`.
...
- 📨 **Cliente:** *"Escreve o **processo da renovação mensal ao check-in liberado**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da renovação mensal ao check-in liberado com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Aluno valida o plano, cobra E libera a catraca — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Aluno valida o plano, cobra E libera a catraca — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: a regra de acesso por tipo de plano cheia de ifs — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem a regra de acesso por tipo de plano cheia de ifs e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de acesso (catraca) e financeiro (planos). Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (acesso (catraca) e financeiro (planos)) e reorganizem o código em `com.academia.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em a catraca na entrada e um servidor com os planos."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (a catraca na entrada e um servidor com os planos) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: fazer check-in na catraca, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "fazer check-in na catraca" do início ao fim e apliquem **um** padrão onde couber (não force).
