# 🚀 Plano de Evolução — 09 — Academia: planos e frequência

> **Este é o roteiro do seu grupo.** O projeto **não nasce pronto**: a cada aula, alguém do
> projeto (o dono de uma academia, gerente, dev sênior…) traz um **novo problema**, e resolvê-lo bem exige
...
- 🎙️ **O que mudou:** o dono de uma academia chega com a ideia inicial: *"Por enquanto, só quero **cadastrar e listar os alunos**. Deixa o resto pra depois — quero ver algo funcionando logo."* Ainda não há regras; é o primeiro esboço do sistema.
...
- 🎙️ **O que mudou:** na segunda conversa, o dono de uma academia completa: *"Ah, esqueci — também preciso registrar os check-ins de treino, e um aluno acumula vários por mês."* De repente o programa do Encontro 1 fica confuso: os mesmos dados se repetem e se espalham por várias listas.
- 💡 **Conceito da aula:** objeto = estado + comportamento; como identificar entidades.
- 🎯 **O que fazer no seu projeto:** releiam o briefing + esse novo pedido e **grifem**: cada substantivo importante (aluno, check-in…) vira candidato a **classe**; cada característica, a **atributo**; cada verbo, a **operação**. Montem a tabela "termo → classe / atributo / operação".
...
- 🎙️ **O que mudou:** o **dev sênior** revisa o código do Encontro 1: *"Isso não escala. Cada check-in é uma linha solta numa lista, e a gente se perde. Vamos transformar cada coisa do domínio numa classe de verdade, com construtor."*
...
- 🎙️ **O que mudou:** conversando de novo, o dono de uma academia solta uma regra que ninguém tinha dito: *"Tem uma coisa que não pode acontecer de jeito nenhum: não pode fazer check-in com o plano vencido."* Isso não é um dado — é um **comportamento** que o sistema precisa garantir.
- 💡 **Conceito da aula:** atributos (visibilidade) e operações (assinatura).
- 🎯 **O que fazer no seu projeto:** definam a **visibilidade** de cada atributo e a **assinatura** de cada operação (parâmetros e retorno). Decidam **qual classe** será dona da regra "não pode fazer check-in com o plano vencido" e criem a operação que a implementa.
...
- 🎙️ **O que mudou:** o **dev sênior** chega preocupado com um incidente: *"Tivemos um problema sério: o valor do plano ficou negativo. Alguém alterou o dado direto, sem passar por validação. Não pode se repetir."*
- 💡 **Conceito da aula:** `private` + validação; proteger o invariante.
- 🎯 **O que fazer no seu projeto:** deixem os atributos `private` e criem métodos que **validam antes de alterar** o estado. Implementem em código a regra "não pode fazer check-in com o plano vencido": se a operação a violaria, ela **recusa** (lança exceção) em vez de gravar o valor errado.
...
- 🎙️ **O que mudou:** o **analista** aponta: *"Precisamos enxergar como as coisas se conectam — um aluno tem um plano ativo e muitos check-ins — e quantas de cada lado."* As classes deixam de ser ilhas.
- 💡 **Conceito da aula:** associação, multiplicidade, agregação × composição.
- 🎯 **O que fazer no seu projeto:** liguem suas classes com a **multiplicidade nas duas pontas** (um aluno tem um plano ativo e muitos check-ins). Para cada "tem-um", decidam: a parte **morre com o todo** (composição ◆) ou **sobrevive** (agregação ◇)?
...
- 🎙️ **O que mudou:** o dono de uma academia traz uma novidade de negócio: *"Passamos a ter um **aluno com plano família**. É quase igual aos outros alunos, mas com um detalhe: divide a mensalidade entre dependentes."* Copiar e colar a classe inteira só pra mudar um detalhe seria desperdício (e fonte de bug).
- 💡 **Conceito da aula:** `extends`, `super`, reúso (o "é um").
- 🎯 **O que fazer no seu projeto:** vocês já têm a classe **`aluno`**. Criem uma **subclasse** para o **aluno com plano família** que **herda** de `aluno` e **acrescenta só o que muda** (divide a mensalidade entre dependentes) — não reescrevam o que já existe, reaproveitem com `super`. No `main`, criem um aluno com plano família e usem-no onde um `aluno` é esperado.
- 📦 **Entregável:** subclasse de `aluno` funcionando, **sem duplicar** o código da base.
- 🔒 **Restrição nova:** só herdem se passar no teste **"é um"** (aluno com plano família **é um** aluno?); se for "tem um", usem composição. A superclasse pode ser `abstract`.
...
- 🎙️ **O que mudou:** o **dev sênior** sente um cheiro ruim no código: *"Vi vários `if` verificando o tipo do aluno pra tratar o aluno com plano família de um jeito diferente. Isso vira um pesadelo quando aparecer mais um tipo."*
- 💡 **Conceito da aula:** sobrescrita (`@Override`), ligação dinâmica.
- 🎯 **O que fazer no seu projeto:** peguem o comportamento que muda entre os tipos (ex.: divide a mensalidade entre dependentes) e **sobrescrevam** esse método na subclasse aluno com plano família. No resto do sistema, chamem o método pela **referência de `aluno`** e deixem o objeto decidir o que fazer.
...
- 🎯 **O que fazer no seu projeto:** identifiquem os **atores** (quem usa o sistema) e listem os **casos de uso** (verbo + objeto). Desenhem o diagrama e escrevam o **fluxo principal + 1 alternativo** do caso mais importante (fazer check-in na catraca).
...
- 🎙️ **O que mudou:** o **dev sênior** antecipa uma mudança: *"Hoje só temos um jeito de lidar com a forma de pagamento da mensalidade (cartão recorrente, Pix), mas isso vai mudar em breve. Não podem amarrar o sistema a uma implementação só."*
- 💡 **Conceito da aula:** classe abstrata + interface (contrato).
- 🎯 **O que fazer no seu projeto:** criem a interface **`MeioDePagamento`** para a forma de pagamento da mensalidade (cartão recorrente, Pix), com pelo menos **duas implementações** diferentes. Façam o resto do sistema depender **só da interface** e escolher a implementação na hora de criar o objeto.
...
- 🎙️ **O que mudou:** o **analista de qualidade** quer validar o comportamento: *"Me mostra, passo a passo, o que acontece quando alguém vai fazer check-in na catraca. Quero ver **quem chama quem** e o que acontece se algo der errado."*
- 💡 **Conceito da aula:** objetos concretos + mensagens no tempo.
- 🎯 **O que fazer no seu projeto:** desenhem a **sequência** de "fazer check-in na catraca", mostrando as mensagens entre os objetos na ordem em que acontecem, incluindo o **caminho alternativo** (por ex., quando a regra "não pode fazer check-in com o plano vencido" é violada).
...
- 🎙️ **O que mudou:** o **analista** percebe um problema recorrente: *"O plano está bagunçado — teve caso de **pular etapa**. Precisa ter fases bem definidas e não deixar avançar fora de ordem."*
- 💡 **Conceito da aula:** estados, transições, guardas.
- 🎯 **O que fazer no seu projeto:** mapeiem os estados de o plano (`ativo → vencido → ativo (após pagamento); ou cancelado`) e o evento que dispara cada transição. Depois, no código, garantam que só dá para ir de um estado ao **próximo válido**.
- 📦 **Entregável:** máquina de estados de o plano + as transições implementadas como métodos.
...
- 🎙️ **O que mudou:** o dono de uma academia quer treinar a equipe nova: *"Preciso do **passo a passo da renovação mensal ao check-in liberado**, de um jeito que qualquer funcionário entenda o processo."*
- 💡 **Conceito da aula:** fluxo de processo, decisões, raias.
- 🎯 **O que fazer no seu projeto:** modelem o processo da renovação mensal ao check-in liberado com início, ações, **decisões** e fim; usem **raias** para deixar claro **quem** faz cada passo (cliente, atendente, sistema).
...
- 🎙️ **O que mudou:** o **tech lead** dá o alerta numa revisão: *"O sistema cresceu e virou uma bola de lama — toda mudancinha quebra outra coisa. Por exemplo: a classe Aluno valida o plano, cobra E libera a catraca. Precisamos organizar."*
- 💡 **Conceito da aula:** SRP (responsabilidade única), OCP (aberto/fechado)…
- 🎯 **O que fazer no seu projeto:** achem a classe que "faz demais" e **separem responsabilidades** — comecem por: a classe Aluno valida o plano, cobra E libera a catraca. Deixem a **regra de negócio** numa classe e a parte de **tela/arquivo/e-mail** em outra.
...
- 🎙️ **O que mudou:** o **tech lead** revisa de novo: *"Agora exageraram pro outro lado — a regra de acesso por tipo de plano cheia de ifs — e ainda tem código que ninguém usa. **Simplifiquem.**"*
- 💡 **Conceito da aula:** KISS (simplicidade), DRY (não repita), YAGNI (não faça o que não pediram).
- 🎯 **O que fazer no seu projeto:** removam duplicação (extraiam método), simplifiquem a regra de acesso por tipo de plano cheia de ifs e **apaguem** todo código que nenhum requisito atual pede.
...
- 🎙️ **O que mudou:** o **arquiteto** planeja crescer o time: *"Outro time vai assumir a parte de acesso (catraca) e financeiro (planos). Precisamos separar isso em **módulos** com fronteiras claras."*
- 💡 **Conceito da aula:** componentes (interfaces) + pacotes (camadas, sem ciclos).
- 🎯 **O que fazer no seu projeto:** agrupem as classes em **pacotes por responsabilidade** e reorganizem o código em `com.academia.{dominio,aplicacao,infra}`. Desenhem as dependências entre os pacotes.
...
- 🎙️ **O que mudou:** o pessoal de **infra (DevOps)** entra na conversa: *"Antes de publicar, preciso saber **onde** isso vai rodar. Pensei em a catraca na entrada e um servidor com os planos. Como as partes se conectam?"*
- 💡 **Conceito da aula:** nós (hardware/ambiente), artefatos, conexões de rede.
- 🎯 **O que fazer no seu projeto:** desenhem onde cada parte roda (a catraca na entrada e um servidor com os planos) e rotulem o **protocolo** de cada conexão (HTTP, JDBC…). Marquem o que é **público** e o que é **interno**.
...
- 🎙️ **O que mudou:** o dono de uma academia e o **gerente** fecham o ciclo: *"Chegou a hora: quero ver **rodando de ponta a ponta** — fazer check-in na catraca — do jeito que a gente combinou."*
- 💡 **Conceito da aula:** integração + um padrão simples (Factory, Strategy).
- 🎯 **O que fazer no seu projeto:** montem um `Principal` que executa "fazer check-in na catraca" do começo ao fim, usando as classes de verdade. Se um padrão simples ajudar (ex.: *Factory* para criar objetos, *Strategy* para a forma de pagamento da mensalidade (cartão recorrente, Pix)), apliquem — **sem forçar**.
