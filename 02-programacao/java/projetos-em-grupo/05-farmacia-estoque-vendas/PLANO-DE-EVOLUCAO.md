...
> (dono de uma farmácia de bairro) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os medicamentos**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista produtos em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar as vendas, e cada venda tem vários itens."*
...
- 📨 **Cliente (falando demais):** *"o produto torce pro tal time, faz aniversário em maio, indicou um amigo…"* — 90% irrelevante.
...
- 📨 **Cliente:** *"Quero que cada venda seja uma 'coisa' de verdade no sistema, não uma linha solta."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode vender um item sem estoque suficiente."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode vender um item sem estoque suficiente" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: a quantidade em estoque ficou negativa. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode vender um item sem estoque suficiente": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: uma venda é feita de vários itens; cada item aponta para um produto — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (uma venda é feita de vários itens; cada item aponta para um produto) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: medicamento controlado (exige receita)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **medicamento controlado (exige receita)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse medicamento controlado (exige receita) **se comporta diferente**: só pode ser vendido com receita registrada."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **medicamento controlado (exige receita)** faça o seu (só pode ser vendido com receita registrada) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de pagamento (dinheiro, cartão, convênio) sem refazer o sistema."*
...
- 🎯 **Direcionamento:** criem a interface `MeioDePagamento` para a forma de pagamento (dinheiro, cartão, convênio) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece registrar uma venda com baixa no estoque."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "registrar uma venda com baixa no estoque" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📨 **Cliente:** *"A venda passa por **fases** e **não pode pular etapa**."*
- 💡 **Exercita:** estados, transições, **guardas** (regra de negócio na transição).
- 📦 **Entregável:** **máquina de estados** de a venda: `aberta → paga → concluída; ou cancelada`.
...
- 📨 **Cliente:** *"Escreve o **processo da chegada do cliente à baixa no estoque**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da chegada do cliente à baixa no estoque com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Venda dá baixa no estoque, calcula o total E emite o cupom — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Venda dá baixa no estoque, calcula o total E emite o cupom — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: a regra de desconto com muitos casos especiais — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem a regra de desconto com muitos casos especiais e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de vendas (PDV) e controle de estoque. Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (vendas (PDV) e controle de estoque) e reorganizem o código em `com.farmacia.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em o caixa (PDV) e um servidor com o banco do estoque."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (o caixa (PDV) e um servidor com o banco do estoque) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: registrar uma venda com baixa no estoque, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "registrar uma venda com baixa no estoque" do início ao fim e apliquem **um** padrão onde couber (não force).
