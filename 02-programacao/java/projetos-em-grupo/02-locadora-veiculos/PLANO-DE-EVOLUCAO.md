...
> (dono de uma locadora de carros) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os veículos**. Nada de mais."*
...
- 📨 **Cliente:** *"Ah, também preciso registrar as locações de cada veículo, e um cliente pode ter várias ao longo do tempo."*
...
- 📨 **Cliente:** *"Quero que cada locação seja uma 'coisa' de verdade no sistema, não uma linha solta."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode alugar um veículo que já está locado no período."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode alugar um veículo que já está locado no período" numa operação.
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode alugar um veículo que já está locado no período": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: um cliente faz várias locações; cada locação é de um veículo — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (um cliente faz várias locações; cada locação é de um veículo) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: cliente premium (aluga com frequência)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **cliente premium (aluga com frequência)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse cliente premium (aluga com frequência) **se comporta diferente**: tem desconto na diária e prioridade na reserva."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **cliente premium (aluga com frequência)** faça o seu (tem desconto na diária e prioridade na reserva) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de pagamento (cartão, Pix) sem refazer o sistema."*
...
- 🎯 **Direcionamento:** criem a interface `MeioDePagamento` para a forma de pagamento (cartão, Pix) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece alugar um carro (retirada)."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "alugar um carro (retirada)" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📨 **Cliente:** *"O veículo passa por **fases** e **não pode pular etapa**."*
- 💡 **Exercita:** estados, transições, **guardas** (regra de negócio na transição).
- 📦 **Entregável:** **máquina de estados** de o veículo: `disponível → alugado → disponível; ou em manutenção`.
...
- 📨 **Cliente:** *"Escreve o **processo da retirada à devolução do veículo**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da retirada à devolução do veículo com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Locacao calcula o valor, gera o contrato E registra a multa — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Locacao calcula o valor, gera o contrato E registra a multa — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: o cálculo de multa por atraso cheio de casos especiais — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem o cálculo de multa por atraso cheio de casos especiais e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de pagamento e emissão de contrato. Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (pagamento e emissão de contrato) e reorganizem o código em `com.locadora.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em um computador no balcão e um site para reserva online."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (um computador no balcão e um site para reserva online) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: alugar um carro (retirada), do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "alugar um carro (retirada)" do início ao fim e apliquem **um** padrão onde couber (não force).
