# 🚀 Plano de Evolução — 20 — Condomínio: moradores e reservas

> **Este é o roteiro do seu grupo.** O projeto **não nasce pronto**: a cada aula, alguém do
> projeto (o síndico de um condomínio, gerente, dev sênior…) traz um **novo problema**, e resolvê-lo bem exige
...
- 🎙️ **O que mudou:** o síndico de um condomínio chega com a ideia inicial: *"Por enquanto, só quero **cadastrar e listar os moradores e as áreas comuns**. Deixa o resto pra depois — quero ver algo funcionando logo."* Ainda não há regras; é o primeiro esboço do sistema.
- 💡 **Conceito da aula:** variáveis, listas, laços, entrada/saída (ainda **sem classes**).
- 🎯 **O que fazer no seu projeto:** façam um programa de menu que permita **adicionar** e **listar** moradores, guardando tudo numa lista/array dentro do `main`. Só tipos básicos.
- 📦 **Entregável:** programa Java que roda e mostra a lista de moradores cadastrados.
...
- 🎙️ **O que mudou:** na segunda conversa, o síndico de um condomínio completa: *"Ah, esqueci — também preciso registrar as reservas das áreas comuns, e um morador pode ter várias."* De repente o programa do Encontro 1 fica confuso: os mesmos dados se repetem e se espalham por várias listas.
- 💡 **Conceito da aula:** objeto = estado + comportamento; como identificar entidades.
- 🎯 **O que fazer no seu projeto:** releiam o briefing + esse novo pedido e **grifem**: cada substantivo importante (morador, reserva…) vira candidato a **classe**; cada característica, a **atributo**; cada verbo, a **operação**. Montem a tabela "termo → classe / atributo / operação".
...
- 🎙️ **O que mudou:** o **gerente do projeto** entra na conversa: *"O cliente falou um monte de coisa na reunião — até o time de futebol do morador. Nem tudo importa. Foquem no que o sistema precisa de verdade."*
...
- 🎙️ **O que mudou:** o **dev sênior** revisa o código do Encontro 1: *"Isso não escala. Cada reserva é uma linha solta numa lista, e a gente se perde. Vamos transformar cada coisa do domínio numa classe de verdade, com construtor."*
- 💡 **Conceito da aula:** `class`, atributos, construtor, `this`, `new`.
- 🎯 **O que fazer no seu projeto:** transformem as classes candidatas do Encontro 3 em **classes Java**, cada uma com **construtor** que exige os dados obrigatórios. No `main`, criem alguns objetos (ex.: alguns moradores) e usem.
...
- 🎙️ **O que mudou:** conversando de novo, o síndico de um condomínio solta uma regra que ninguém tinha dito: *"Tem uma coisa que não pode acontecer de jeito nenhum: não pode reservar uma área comum já reservada no mesmo período."* Isso não é um dado — é um **comportamento** que o sistema precisa garantir.
- 💡 **Conceito da aula:** atributos (visibilidade) e operações (assinatura).
- 🎯 **O que fazer no seu projeto:** definam a **visibilidade** de cada atributo e a **assinatura** de cada operação (parâmetros e retorno). Decidam **qual classe** será dona da regra "não pode reservar uma área comum já reservada no mesmo período" e criem a operação que a implementa.
...
- 🎙️ **O que mudou:** o **dev sênior** chega preocupado com um incidente: *"Tivemos um problema sério: a taxa de reserva ficou negativa. Alguém alterou o dado direto, sem passar por validação. Não pode se repetir."*
- 💡 **Conceito da aula:** `private` + validação; proteger o invariante.
- 🎯 **O que fazer no seu projeto:** deixem os atributos `private` e criem métodos que **validam antes de alterar** o estado. Implementem em código a regra "não pode reservar uma área comum já reservada no mesmo período": se a operação a violaria, ela **recusa** (lança exceção) em vez de gravar o valor errado.
...
- 🎙️ **O que mudou:** o **analista** aponta: *"Precisamos enxergar como as coisas se conectam — um morador pertence a uma unidade e faz várias reservas de áreas comuns — e quantas de cada lado."* As classes deixam de ser ilhas.
- 💡 **Conceito da aula:** associação, multiplicidade, agregação × composição.
- 🎯 **O que fazer no seu projeto:** liguem suas classes com a **multiplicidade nas duas pontas** (um morador pertence a uma unidade e faz várias reservas de áreas comuns). Para cada "tem-um", decidam: a parte **morre com o todo** (composição ◆) ou **sobrevive** (agregação ◇)?
...
- 🎙️ **O que mudou:** o síndico de um condomínio traz uma novidade de negócio: *"Passamos a ter um **morador inadimplente**. É quase igual aos outros moradors, mas com um detalhe: fica bloqueado de reservar até quitar."* Copiar e colar a classe inteira só pra mudar um detalhe seria desperdício (e fonte de bug).
- 💡 **Conceito da aula:** `extends`, `super`, reúso (o "é um").
- 🎯 **O que fazer no seu projeto:** vocês já têm a classe **`morador`**. Criem uma **subclasse** para o **morador inadimplente** que **herda** de `morador` e **acrescenta só o que muda** (fica bloqueado de reservar até quitar) — não reescrevam o que já existe, reaproveitem com `super`. No `main`, criem um morador inadimplente e usem-no onde um `morador` é esperado.
- 📦 **Entregável:** subclasse de `morador` funcionando, **sem duplicar** o código da base.
- 🔒 **Restrição nova:** só herdem se passar no teste **"é um"** (morador inadimplente **é um** morador?); se for "tem um", usem composição. A superclasse pode ser `abstract`.
...
- 🎙️ **O que mudou:** o **dev sênior** sente um cheiro ruim no código: *"Vi vários `if` verificando o tipo do morador pra tratar o morador inadimplente de um jeito diferente. Isso vira um pesadelo quando aparecer mais um tipo."*
- 💡 **Conceito da aula:** sobrescrita (`@Override`), ligação dinâmica.
- 🎯 **O que fazer no seu projeto:** peguem o comportamento que muda entre os tipos (ex.: fica bloqueado de reservar até quitar) e **sobrescrevam** esse método na subclasse morador inadimplente. No resto do sistema, chamem o método pela **referência de `morador`** e deixem o objeto decidir o que fazer.
...
- 🎯 **O que fazer no seu projeto:** identifiquem os **atores** (quem usa o sistema) e listem os **casos de uso** (verbo + objeto). Desenhem o diagrama e escrevam o **fluxo principal + 1 alternativo** do caso mais importante (reservar uma área comum).
...
- 🎙️ **O que mudou:** o **dev sênior** antecipa uma mudança: *"Hoje só temos um jeito de lidar com a forma de cobrança da taxa (boleto, Pix), mas isso vai mudar em breve. Não podem amarrar o sistema a uma implementação só."*
- 💡 **Conceito da aula:** classe abstrata + interface (contrato).
- 🎯 **O que fazer no seu projeto:** criem a interface **`MeioDeCobranca`** para a forma de cobrança da taxa (boleto, Pix), com pelo menos **duas implementações** diferentes. Façam o resto do sistema depender **só da interface** e escolher a implementação na hora de criar o objeto.
- 📦 **Entregável:** interface `MeioDeCobranca` + 2 implementações intercambiáveis.
...
- 🎙️ **O que mudou:** o **analista de qualidade** quer validar o comportamento: *"Me mostra, passo a passo, o que acontece quando alguém vai reservar uma área comum. Quero ver **quem chama quem** e o que acontece se algo der errado."*
- 💡 **Conceito da aula:** objetos concretos + mensagens no tempo.
- 🎯 **O que fazer no seu projeto:** desenhem a **sequência** de "reservar uma área comum", mostrando as mensagens entre os objetos na ordem em que acontecem, incluindo o **caminho alternativo** (por ex., quando a regra "não pode reservar uma área comum já reservada no mesmo período" é violada).
...
- 🎙️ **O que mudou:** o **analista** percebe um problema recorrente: *"A reserva está bagunçado — teve caso de **pular etapa**. Precisa ter fases bem definidas e não deixar avançar fora de ordem."*
- 💡 **Conceito da aula:** estados, transições, guardas.
- 🎯 **O que fazer no seu projeto:** mapeiem os estados de a reserva (`solicitada → confirmada → concluída; ou cancelada`) e o evento que dispara cada transição. Depois, no código, garantam que só dá para ir de um estado ao **próximo válido**.
- 📦 **Entregável:** máquina de estados de a reserva + as transições implementadas como métodos.
...
- 🎙️ **O que mudou:** o síndico de um condomínio quer treinar a equipe nova: *"Preciso do **passo a passo da solicitação à liberação da área no dia**, de um jeito que qualquer funcionário entenda o processo."*
- 💡 **Conceito da aula:** fluxo de processo, decisões, raias.
- 🎯 **O que fazer no seu projeto:** modelem o processo da solicitação à liberação da área no dia com início, ações, **decisões** e fim; usem **raias** para deixar claro **quem** faz cada passo (cliente, atendente, sistema).
...
- 🎙️ **O que mudou:** o **tech lead** dá o alerta numa revisão: *"O sistema cresceu e virou uma bola de lama — toda mudancinha quebra outra coisa. Por exemplo: a classe Reserva confirma, cobra a taxa E avisa a portaria. Precisamos organizar."*
- 💡 **Conceito da aula:** SRP (responsabilidade única), OCP (aberto/fechado)…
- 🎯 **O que fazer no seu projeto:** achem a classe que "faz demais" e **separem responsabilidades** — comecem por: a classe Reserva confirma, cobra a taxa E avisa a portaria. Deixem a **regra de negócio** numa classe e a parte de **tela/arquivo/e-mail** em outra.
...
- 🎙️ **O que mudou:** o **tech lead** revisa de novo: *"Agora exageraram pro outro lado — a checagem de disponibilidade repetida em vários pontos — e ainda tem código que ninguém usa. **Simplifiquem.**"*
- 💡 **Conceito da aula:** KISS (simplicidade), DRY (não repita), YAGNI (não faça o que não pediram).
- 🎯 **O que fazer no seu projeto:** removam duplicação (extraiam método), simplifiquem a checagem de disponibilidade repetida em vários pontos e **apaguem** todo código que nenhum requisito atual pede.
...
- 🎙️ **O que mudou:** o **arquiteto** planeja crescer o time: *"Outro time vai assumir a parte de reservas e cobrança de taxas. Precisamos separar isso em **módulos** com fronteiras claras."*
- 💡 **Conceito da aula:** componentes (interfaces) + pacotes (camadas, sem ciclos).
- 🎯 **O que fazer no seu projeto:** agrupem as classes em **pacotes por responsabilidade** e reorganizem o código em `com.condominio.{dominio,aplicacao,infra}`. Desenhem as dependências entre os pacotes.
...
- 🎙️ **O que mudou:** o pessoal de **infra (DevOps)** entra na conversa: *"Antes de publicar, preciso saber **onde** isso vai rodar. Pensei em um computador na portaria e um app para os moradores reservarem. Como as partes se conectam?"*
- 💡 **Conceito da aula:** nós (hardware/ambiente), artefatos, conexões de rede.
- 🎯 **O que fazer no seu projeto:** desenhem onde cada parte roda (um computador na portaria e um app para os moradores reservarem) e rotulem o **protocolo** de cada conexão (HTTP, JDBC…). Marquem o que é **público** e o que é **interno**.
...
- 🎙️ **O que mudou:** o síndico de um condomínio e o **gerente** fecham o ciclo: *"Chegou a hora: quero ver **rodando de ponta a ponta** — reservar uma área comum — do jeito que a gente combinou."*
- 💡 **Conceito da aula:** integração + um padrão simples (Factory, Strategy).
- 🎯 **O que fazer no seu projeto:** montem um `Principal` que executa "reservar uma área comum" do começo ao fim, usando as classes de verdade. Se um padrão simples ajudar (ex.: *Factory* para criar objetos, *Strategy* para a forma de cobrança da taxa (boleto, Pix)), apliquem — **sem forçar**.
