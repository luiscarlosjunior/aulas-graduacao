...
> (síndico de um condomínio) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os moradores e as áreas comuns**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista moradores em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar as reservas das áreas comuns, e um morador pode ter várias."*
...
- 📨 **Cliente (falando demais):** *"o morador torce pro tal time, faz aniversário em maio, indicou um amigo…"* — 90% irrelevante.
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode reservar uma área comum já reservada no mesmo período."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode reservar uma área comum já reservada no mesmo período" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: a taxa de reserva ficou negativa. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode reservar uma área comum já reservada no mesmo período": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: um morador pertence a uma unidade e faz várias reservas de áreas comuns — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (um morador pertence a uma unidade e faz várias reservas de áreas comuns) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: morador inadimplente (vs em dia)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **morador inadimplente (vs em dia)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse morador inadimplente (vs em dia) **se comporta diferente**: fica bloqueado de reservar até quitar."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **morador inadimplente (vs em dia)** faça o seu (fica bloqueado de reservar até quitar) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de cobrança da taxa (boleto, Pix) sem refazer o sistema."*
- 💡 **Exercita:** classe abstrata + **interface** = contrato plugável (baixo acoplamento).
- 📦 **Entregável:** uma **interface** `MeioDeCobranca` com **2 implementações** intercambiáveis.
- 🎯 **Direcionamento:** criem a interface `MeioDeCobranca` para a forma de cobrança da taxa (boleto, Pix) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece reservar uma área comum."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "reservar uma área comum" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📦 **Entregável:** **máquina de estados** de a reserva: `solicitada → confirmada → concluída; ou cancelada`.
...
- 📨 **Cliente:** *"Escreve o **processo da solicitação à liberação da área no dia**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da solicitação à liberação da área no dia com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Reserva confirma, cobra a taxa E avisa a portaria — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Reserva confirma, cobra a taxa E avisa a portaria — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: a checagem de disponibilidade repetida em vários pontos — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem a checagem de disponibilidade repetida em vários pontos e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de reservas e cobrança de taxas. Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (reservas e cobrança de taxas) e reorganizem o código em `com.condominio.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em um computador na portaria e um app para os moradores reservarem."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (um computador na portaria e um app para os moradores reservarem) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: reservar uma área comum, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "reservar uma área comum" do início ao fim e apliquem **um** padrão onde couber (não force).
