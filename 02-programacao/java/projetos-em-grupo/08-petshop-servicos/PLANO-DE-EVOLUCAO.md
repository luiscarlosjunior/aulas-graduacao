# 🚀 Plano de Evolução — 08 — Petshop: serviços e agenda

> **Este é o roteiro do seu grupo.** O projeto **não nasce pronto**: a cada aula, alguém do
> projeto (o dono de um petshop, gerente, dev sênior…) traz um **novo problema**, e resolvê-lo bem exige
...
- 🎙️ **O que mudou:** o dono de um petshop chega com a ideia inicial: *"Por enquanto, só quero **cadastrar e listar os donos e seus pets**. Deixa o resto pra depois — quero ver algo funcionando logo."* Ainda não há regras; é o primeiro esboço do sistema.
- 💡 **Conceito da aula:** variáveis, listas, laços, entrada/saída (ainda **sem classes**).
- 🎯 **O que fazer no seu projeto:** façam um programa de menu que permita **adicionar** e **listar** pets, guardando tudo numa lista/array dentro do `main`. Só tipos básicos.
- 📦 **Entregável:** programa Java que roda e mostra a lista de pets cadastrados.
...
- 🎙️ **O que mudou:** na segunda conversa, o dono de um petshop completa: *"Ah, esqueci — também preciso registrar os agendamentos de serviço, e um pet pode ter vários."* De repente o programa do Encontro 1 fica confuso: os mesmos dados se repetem e se espalham por várias listas.
- 💡 **Conceito da aula:** objeto = estado + comportamento; como identificar entidades.
- 🎯 **O que fazer no seu projeto:** releiam o briefing + esse novo pedido e **grifem**: cada substantivo importante (pet, agendamento…) vira candidato a **classe**; cada característica, a **atributo**; cada verbo, a **operação**. Montem a tabela "termo → classe / atributo / operação".
...
- 🎙️ **O que mudou:** o **gerente do projeto** entra na conversa: *"O cliente falou um monte de coisa na reunião — até o time de futebol do pet. Nem tudo importa. Foquem no que o sistema precisa de verdade."*
...
- 🎯 **O que fazer no seu projeto:** transformem as classes candidatas do Encontro 3 em **classes Java**, cada uma com **construtor** que exige os dados obrigatórios. No `main`, criem alguns objetos (ex.: alguns pets) e usem.
...
- 🎙️ **O que mudou:** conversando de novo, o dono de um petshop solta uma regra que ninguém tinha dito: *"Tem uma coisa que não pode acontecer de jeito nenhum: não pode agendar dois serviços no mesmo horário do mesmo profissional."* Isso não é um dado — é um **comportamento** que o sistema precisa garantir.
...
- 🎙️ **O que mudou:** o **analista** aponta: *"Precisamos enxergar como as coisas se conectam — um dono tem vários pets; cada agendamento é de um pet para um serviço — e quantas de cada lado."* As classes deixam de ser ilhas.
- 💡 **Conceito da aula:** associação, multiplicidade, agregação × composição.
- 🎯 **O que fazer no seu projeto:** liguem suas classes com a **multiplicidade nas duas pontas** (um dono tem vários pets; cada agendamento é de um pet para um serviço). Para cada "tem-um", decidam: a parte **morre com o todo** (composição ◆) ou **sobrevive** (agregação ◇)?
...
- 🎙️ **O que mudou:** o dono de um petshop traz uma novidade de negócio: *"Passamos a ter um **cliente com pacote mensal**. É quase igual aos outros clientes, mas com um detalhe: tem banhos inclusos e prioridade na agenda."* Copiar e colar a classe inteira só pra mudar um detalhe seria desperdício (e fonte de bug).
- 💡 **Conceito da aula:** `extends`, `super`, reúso (o "é um").
- 🎯 **O que fazer no seu projeto:** vocês já têm a classe **`cliente`**. Criem uma **subclasse** para o **cliente com pacote mensal** que **herda** de `cliente` e **acrescenta só o que muda** (tem banhos inclusos e prioridade na agenda) — não reescrevam o que já existe, reaproveitem com `super`. No `main`, criem um cliente com pacote mensal e usem-no onde um `cliente` é esperado.
- 📦 **Entregável:** subclasse de `cliente` funcionando, **sem duplicar** o código da base.
- 🔒 **Restrição nova:** só herdem se passar no teste **"é um"** (cliente com pacote mensal **é um** cliente?); se for "tem um", usem composição. A superclasse pode ser `abstract`.
...
- 🎙️ **O que mudou:** o **dev sênior** sente um cheiro ruim no código: *"Vi vários `if` verificando o tipo do cliente pra tratar o cliente com pacote mensal de um jeito diferente. Isso vira um pesadelo quando aparecer mais um tipo."*
- 💡 **Conceito da aula:** sobrescrita (`@Override`), ligação dinâmica.
- 🎯 **O que fazer no seu projeto:** peguem o comportamento que muda entre os tipos (ex.: tem banhos inclusos e prioridade na agenda) e **sobrescrevam** esse método na subclasse cliente com pacote mensal. No resto do sistema, chamem o método pela **referência de `cliente`** e deixem o objeto decidir o que fazer.
...
- 🎯 **O que fazer no seu projeto:** identifiquem os **atores** (quem usa o sistema) e listem os **casos de uso** (verbo + objeto). Desenhem o diagrama e escrevam o **fluxo principal + 1 alternativo** do caso mais importante (agendar um serviço para um pet).
...
- 🎙️ **O que mudou:** o **dev sênior** antecipa uma mudança: *"Hoje só temos um jeito de lidar com a forma de pagamento (avulso, pacote, assinatura), mas isso vai mudar em breve. Não podem amarrar o sistema a uma implementação só."*
- 💡 **Conceito da aula:** classe abstrata + interface (contrato).
- 🎯 **O que fazer no seu projeto:** criem a interface **`MeioDePagamento`** para a forma de pagamento (avulso, pacote, assinatura), com pelo menos **duas implementações** diferentes. Façam o resto do sistema depender **só da interface** e escolher a implementação na hora de criar o objeto.
...
- 🎙️ **O que mudou:** o **analista de qualidade** quer validar o comportamento: *"Me mostra, passo a passo, o que acontece quando alguém vai agendar um serviço para um pet. Quero ver **quem chama quem** e o que acontece se algo der errado."*
- 💡 **Conceito da aula:** objetos concretos + mensagens no tempo.
- 🎯 **O que fazer no seu projeto:** desenhem a **sequência** de "agendar um serviço para um pet", mostrando as mensagens entre os objetos na ordem em que acontecem, incluindo o **caminho alternativo** (por ex., quando a regra "não pode agendar dois serviços no mesmo horário do mesmo profissional" é violada).
...
- 🎯 **O que fazer no seu projeto:** mapeiem os estados de o agendamento (`agendado → em atendimento → concluído; ou cancelado/faltou`) e o evento que dispara cada transição. Depois, no código, garantam que só dá para ir de um estado ao **próximo válido**.
...
- 🎙️ **O que mudou:** o dono de um petshop quer treinar a equipe nova: *"Preciso do **passo a passo da chegada do pet à entrega para o dono**, de um jeito que qualquer funcionário entenda o processo."*
- 💡 **Conceito da aula:** fluxo de processo, decisões, raias.
- 🎯 **O que fazer no seu projeto:** modelem o processo da chegada do pet à entrega para o dono com início, ações, **decisões** e fim; usem **raias** para deixar claro **quem** faz cada passo (cliente, atendente, sistema).
...
- 🎙️ **O que mudou:** o **tech lead** dá o alerta numa revisão: *"O sistema cresceu e virou uma bola de lama — toda mudancinha quebra outra coisa. Por exemplo: a classe Agendamento cobra, agenda E manda o lembrete. Precisamos organizar."*
- 💡 **Conceito da aula:** SRP (responsabilidade única), OCP (aberto/fechado)…
- 🎯 **O que fazer no seu projeto:** achem a classe que "faz demais" e **separem responsabilidades** — comecem por: a classe Agendamento cobra, agenda E manda o lembrete. Deixem a **regra de negócio** numa classe e a parte de **tela/arquivo/e-mail** em outra.
...
- 🎙️ **O que mudou:** o **tech lead** revisa de novo: *"Agora exageraram pro outro lado — a checagem de horário livre repetida em vários pontos — e ainda tem código que ninguém usa. **Simplifiquem.**"*
- 💡 **Conceito da aula:** KISS (simplicidade), DRY (não repita), YAGNI (não faça o que não pediram).
- 🎯 **O que fazer no seu projeto:** removam duplicação (extraiam método), simplifiquem a checagem de horário livre repetida em vários pontos e **apaguem** todo código que nenhum requisito atual pede.
...
- 🎙️ **O que mudou:** o **arquiteto** planeja crescer o time: *"Outro time vai assumir a parte de agenda e financeiro. Precisamos separar isso em **módulos** com fronteiras claras."*
- 💡 **Conceito da aula:** componentes (interfaces) + pacotes (camadas, sem ciclos).
- 🎯 **O que fazer no seu projeto:** agrupem as classes em **pacotes por responsabilidade** e reorganizem o código em `com.petshop.{dominio,aplicacao,infra}`. Desenhem as dependências entre os pacotes.
...
- 🎙️ **O que mudou:** o pessoal de **infra (DevOps)** entra na conversa: *"Antes de publicar, preciso saber **onde** isso vai rodar. Pensei em um computador na recepção e um app para o dono agendar. Como as partes se conectam?"*
- 💡 **Conceito da aula:** nós (hardware/ambiente), artefatos, conexões de rede.
- 🎯 **O que fazer no seu projeto:** desenhem onde cada parte roda (um computador na recepção e um app para o dono agendar) e rotulem o **protocolo** de cada conexão (HTTP, JDBC…). Marquem o que é **público** e o que é **interno**.
...
- 🎙️ **O que mudou:** o dono de um petshop e o **gerente** fecham o ciclo: *"Chegou a hora: quero ver **rodando de ponta a ponta** — agendar um serviço para um pet — do jeito que a gente combinou."*
- 💡 **Conceito da aula:** integração + um padrão simples (Factory, Strategy).
- 🎯 **O que fazer no seu projeto:** montem um `Principal` que executa "agendar um serviço para um pet" do começo ao fim, usando as classes de verdade. Se um padrão simples ajudar (ex.: *Factory* para criar objetos, *Strategy* para a forma de pagamento (avulso, pacote, assinatura)), apliquem — **sem forçar**.
