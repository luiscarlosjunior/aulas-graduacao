...
> (dono de um petshop) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os donos e seus pets**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista pets em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar os agendamentos de serviço, e um pet pode ter vários."*
...
- 📨 **Cliente (falando demais):** *"o pet torce pro tal time, faz aniversário em maio, indicou um amigo…"* — 90% irrelevante.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: um dono tem vários pets; cada agendamento é de um pet para um serviço — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (um dono tem vários pets; cada agendamento é de um pet para um serviço) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: cliente com pacote mensal (vs avulso)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **cliente com pacote mensal (vs avulso)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse cliente com pacote mensal (vs avulso) **se comporta diferente**: tem banhos inclusos e prioridade na agenda."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **cliente com pacote mensal (vs avulso)** faça o seu (tem banhos inclusos e prioridade na agenda) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de pagamento (avulso, pacote, assinatura) sem refazer o sistema."*
...
- 🎯 **Direcionamento:** criem a interface `MeioDePagamento` para a forma de pagamento (avulso, pacote, assinatura) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece agendar um serviço para um pet."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "agendar um serviço para um pet" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📦 **Entregável:** **máquina de estados** de o agendamento: `agendado → em atendimento → concluído; ou cancelado/faltou`.
...
- 📨 **Cliente:** *"Escreve o **processo da chegada do pet à entrega para o dono**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da chegada do pet à entrega para o dono com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Agendamento cobra, agenda E manda o lembrete — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Agendamento cobra, agenda E manda o lembrete — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: a checagem de horário livre repetida em vários pontos — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem a checagem de horário livre repetida em vários pontos e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de agenda e financeiro. Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (agenda e financeiro) e reorganizem o código em `com.petshop.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em um computador na recepção e um app para o dono agendar."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (um computador na recepção e um app para o dono agendar) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: agendar um serviço para um pet, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "agendar um serviço para um pet" do início ao fim e apliquem **um** padrão onde couber (não force).
