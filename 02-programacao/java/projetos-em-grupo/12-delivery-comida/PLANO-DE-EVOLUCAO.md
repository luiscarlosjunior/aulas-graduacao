...
> (dono de um app de delivery) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os restaurantes e os clientes**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista restaurantes em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar os pedidos dos clientes, e cada pedido tem vários itens."*
...
- 📨 **Cliente (falando demais):** *"o restaurante torce pro tal time, faz aniversário em maio, indicou um amigo…"* — 90% irrelevante.
...
- 📨 **Cliente:** *"Quero que cada pedido seja uma 'coisa' de verdade no sistema, não uma linha solta."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode atribuir dois entregadores ao mesmo pedido."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode atribuir dois entregadores ao mesmo pedido" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: a taxa de entrega ficou negativa. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode atribuir dois entregadores ao mesmo pedido": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: um pedido é de um cliente e um restaurante, e é levado por um entregador — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (um pedido é de um cliente e um restaurante, e é levado por um entregador) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: cliente assinante (vs avulso)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **cliente assinante (vs avulso)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse cliente assinante (vs avulso) **se comporta diferente**: tem entrega grátis acima de um valor."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **cliente assinante (vs avulso)** faça o seu (tem entrega grátis acima de um valor) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de pagamento (na entrega ou online) sem refazer o sistema."*
...
- 🎯 **Direcionamento:** criem a interface `MeioDePagamento` para a forma de pagamento (na entrega ou online) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece fazer um pedido até a entrega."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "fazer um pedido até a entrega" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📨 **Cliente:** *"O pedido passa por **fases** e **não pode pular etapa**."*
- 💡 **Exercita:** estados, transições, **guardas** (regra de negócio na transição).
- 📦 **Entregável:** **máquina de estados** de o pedido: `recebido → em preparo → saiu para entrega → entregue; ou cancelado`.
...
- 📨 **Cliente:** *"Escreve o **processo do pedido do cliente à entrega na porta**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem do pedido do cliente à entrega na porta com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Pedido calcula o total, escolhe o entregador E notifica — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Pedido calcula o total, escolhe o entregador E notifica — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: a lógica de taxa de entrega por distância cheia de casos — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem a lógica de taxa de entrega por distância cheia de casos e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de pedidos e logística de entrega. Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (pedidos e logística de entrega) e reorganizem o código em `com.delivery.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em o app do cliente, o app do entregador, um servidor e um banco."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (o app do cliente, o app do entregador, um servidor e um banco) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: fazer um pedido até a entrega, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "fazer um pedido até a entrega" do início ao fim e apliquem **um** padrão onde couber (não force).
