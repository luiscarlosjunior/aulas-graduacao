...
> (dono de uma loja online) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os produtos e os clientes**. Nada de mais."*
...
- 📨 **Cliente:** *"Ah, também preciso registrar os pedidos, e cada pedido tem vários itens no carrinho."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode finalizar um pedido com produto sem estoque."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode finalizar um pedido com produto sem estoque" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: o preço do produto ficou negativo. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode finalizar um pedido com produto sem estoque": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: um cliente faz muitos pedidos; um pedido é feito de vários itens — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (um cliente faz muitos pedidos; um pedido é feito de vários itens) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: cliente VIP (vs comum)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **cliente VIP (vs comum)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse cliente VIP (vs comum) **se comporta diferente**: tem frete grátis e desconto progressivo."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **cliente VIP (vs comum)** faça o seu (tem frete grátis e desconto progressivo) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de pagamento (cartão, Pix, boleto) sem refazer o sistema."*
...
- 🎯 **Direcionamento:** criem a interface `MeioDePagamento` para a forma de pagamento (cartão, Pix, boleto) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece fechar o carrinho (checkout)."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "fechar o carrinho (checkout)" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📦 **Entregável:** **máquina de estados** de o pedido: `criado → pago → enviado → entregue; ou cancelado`.
...
- 📨 **Cliente:** *"Escreve o **processo do carrinho à entrega**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem do carrinho à entrega com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Pedido calcula o total, cobra E dispara o e-mail — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Pedido calcula o total, cobra E dispara o e-mail — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: o cálculo de frete/desconto/cupom embolado — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem o cálculo de frete/desconto/cupom embolado e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de pagamento e envio (logística). Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (pagamento e envio (logística)) e reorganizem o código em `com.loja.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em o site, um servidor de aplicação, um banco e um gateway de pagamento."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (o site, um servidor de aplicação, um banco e um gateway de pagamento) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: fechar o carrinho (checkout), do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "fechar o carrinho (checkout)" do início ao fim e apliquem **um** padrão onde couber (não force).
