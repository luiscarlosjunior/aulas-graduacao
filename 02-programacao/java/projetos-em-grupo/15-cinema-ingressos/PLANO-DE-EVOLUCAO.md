...
> (dono de um cinema) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os filmes**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista filmes em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar as sessões e a venda de ingressos, e cada sessão vende vários ingressos."*
...
- 📨 **Cliente (falando demais):** *"o filme torce pro tal time, faz aniversário em maio, indicou um amigo…"* — 90% irrelevante.
...
- 📨 **Cliente:** *"Quero que cada sessão seja uma 'coisa' de verdade no sistema, não uma linha solta."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode vender o mesmo assento duas vezes na mesma sessão."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode vender o mesmo assento duas vezes na mesma sessão" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: o preço do ingresso ficou negativo. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode vender o mesmo assento duas vezes na mesma sessão": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: uma sessão tem muitos assentos; cada ingresso reserva um assento de uma sessão — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (uma sessão tem muitos assentos; cada ingresso reserva um assento de uma sessão) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: ingresso meia-entrada (vs inteira)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **ingresso meia-entrada (vs inteira)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse ingresso meia-entrada (vs inteira) **se comporta diferente**: custa metade e exige comprovação."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **ingresso meia-entrada (vs inteira)** faça o seu (custa metade e exige comprovação) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de pagamento (cartão, Pix, dinheiro) sem refazer o sistema."*
...
- 🎯 **Direcionamento:** criem a interface `MeioDePagamento` para a forma de pagamento (cartão, Pix, dinheiro) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece comprar um ingresso escolhendo o assento."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "comprar um ingresso escolhendo o assento" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📨 **Cliente:** *"O assento (na sessão) passa por **fases** e **não pode pular etapa**."*
- 💡 **Exercita:** estados, transições, **guardas** (regra de negócio na transição).
- 📦 **Entregável:** **máquina de estados** de o assento (na sessão): `livre → reservado → vendido`.
...
- 📨 **Cliente:** *"Escreve o **processo da escolha da sessão à emissão do ingresso**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da escolha da sessão à emissão do ingresso com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Sessao vende, calcula o valor E imprime o ingresso — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Sessao vende, calcula o valor E imprime o ingresso — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: a regra de preço (meia, promoção, feriado) cheia de ifs — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem a regra de preço (meia, promoção, feriado) cheia de ifs e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de bilheteria e pagamento. Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (bilheteria e pagamento) e reorganizem o código em `com.cinema.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em os totens de autoatendimento, a bilheteria e um servidor."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (os totens de autoatendimento, a bilheteria e um servidor) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: comprar um ingresso escolhendo o assento, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "comprar um ingresso escolhendo o assento" do início ao fim e apliquem **um** padrão onde couber (não force).
