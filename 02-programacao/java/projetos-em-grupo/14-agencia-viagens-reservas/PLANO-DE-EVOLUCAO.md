...
> (dono de uma agência de viagens) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os pacotes**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista pacotes em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar as reservas dos clientes, e um pacote recebe várias reservas."*
...
- 📨 **Cliente (falando demais):** *"o pacote torce pro tal time, faz aniversário em maio, indicou um amigo…"* — 90% irrelevante.
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode reservar mais vagas do que o pacote oferece."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode reservar mais vagas do que o pacote oferece" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: o número de vagas ficou negativo. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode reservar mais vagas do que o pacote oferece": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: um pacote tem muitas reservas; cada reserva é de um cliente — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (um pacote tem muitas reservas; cada reserva é de um cliente) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: cliente com pacote premium (vs econômico)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **cliente com pacote premium (vs econômico)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse cliente com pacote premium (vs econômico) **se comporta diferente**: inclui traslado e seguro-viagem."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **cliente com pacote premium (vs econômico)** faça o seu (inclui traslado e seguro-viagem) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de pagamento (à vista, parcelado) sem refazer o sistema."*
...
- 🎯 **Direcionamento:** criem a interface `MeioDePagamento` para a forma de pagamento (à vista, parcelado) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece reservar um pacote."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "reservar um pacote" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📦 **Entregável:** **máquina de estados** de a reserva: `pendente → confirmada → concluída; ou cancelada`.
...
- 📨 **Cliente:** *"Escreve o **processo da escolha do pacote à emissão do voucher**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da escolha do pacote à emissão do voucher com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Reserva calcula o preço, cobra E emite o voucher — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Reserva calcula o preço, cobra E emite o voucher — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: o cálculo de preço com temporada/desconto/parcelamento confuso — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem o cálculo de preço com temporada/desconto/parcelamento confuso e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de pagamento e emissão de vouchers. Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (pagamento e emissão de vouchers) e reorganizem o código em `com.viagens.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em um site de reservas, um servidor e um banco."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (um site de reservas, um servidor e um banco) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: reservar um pacote, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "reservar um pacote" do início ao fim e apliquem **um** padrão onde couber (não force).
