...
> (dono de uma pousada) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os hóspedes**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista hóspedes em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar as reservas de cada hóspede, e cada hóspede pode ter várias."*
...
- 📨 **Cliente (falando demais):** *"o hóspede torce pro tal time, faz aniversário em maio, indicou um amigo…"* — 90% irrelevante.
...
- 📨 **Cliente:** *"Quero que cada reserva seja uma 'coisa' de verdade no sistema, não uma linha solta."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode existir reserva de um quarto já ocupado nas mesmas datas."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode existir reserva de um quarto já ocupado nas mesmas datas" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: o valor da diária ficou negativo. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode existir reserva de um quarto já ocupado nas mesmas datas": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: um hóspede faz várias reservas; cada reserva é de um quarto — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (um hóspede faz várias reservas; cada reserva é de um quarto) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: hóspede VIP (fidelidade)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **hóspede VIP (fidelidade)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse hóspede VIP (fidelidade) **se comporta diferente**: tem late check-out e isenção da taxa de serviço."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **hóspede VIP (fidelidade)** faça o seu (tem late check-out e isenção da taxa de serviço) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de pagamento da estadia (cartão, Pix, dinheiro) sem refazer o sistema."*
- 💡 **Exercita:** classe abstrata + **interface** = contrato plugável (baixo acoplamento).
- 📦 **Entregável:** uma **interface** `MeioDePagamento` com **2 implementações** intercambiáveis.
- 🎯 **Direcionamento:** criem a interface `MeioDePagamento` para a forma de pagamento da estadia (cartão, Pix, dinheiro) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece fazer uma reserva e o check-in."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "fazer uma reserva e o check-in" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📨 **Cliente:** *"A reserva passa por **fases** e **não pode pular etapa**."*
- 💡 **Exercita:** estados, transições, **guardas** (regra de negócio na transição).
- 📦 **Entregável:** **máquina de estados** de a reserva: `reservada → check-in feito → check-out feito; ou cancelada`.
...
- 📨 **Cliente:** *"Escreve o **processo da chegada do hóspede até o check-out**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da chegada do hóspede até o check-out com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Reserva calcula o valor, emite a fatura E manda o e-mail — separe cada responsabilidade.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Reserva calcula o valor, emite a fatura E manda o e-mail — separe cada responsabilidade.
...
- 📦 **Entregável:** uma **simplificação** — ex.: o cálculo da diária com dezenas de ifs de temporada — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem o cálculo da diária com dezenas de ifs de temporada e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de pagamento e relatório de ocupação. Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (pagamento e relatório de ocupação) e reorganizem o código em `com.hotel.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em um computador na recepção e, quem sabe, um app para o hóspede ver a reserva."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (um computador na recepção e, quem sabe, um app para o hóspede ver a reserva) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: fazer uma reserva e o check-in, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "fazer uma reserva e o check-in" do início ao fim e apliquem **um** padrão onde couber (não force).
