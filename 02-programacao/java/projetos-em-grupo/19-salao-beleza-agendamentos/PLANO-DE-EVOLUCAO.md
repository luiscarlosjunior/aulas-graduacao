...
> (dono de um salão de beleza) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os clientes e os profissionais**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista clientes em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar os agendamentos, e uma cliente pode ter vários ao longo do tempo."*
...
- 📨 **Cliente:** *"Quero que cada agendamento seja uma 'coisa' de verdade no sistema, não uma linha solta."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode agendar dois serviços no mesmo horário do mesmo profissional."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode agendar dois serviços no mesmo horário do mesmo profissional" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: o preço do serviço ficou negativo. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode agendar dois serviços no mesmo horário do mesmo profissional": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: um profissional tem muitos agendamentos; cada agendamento é de uma cliente para um serviço — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (um profissional tem muitos agendamentos; cada agendamento é de uma cliente para um serviço) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: cliente com plano de fidelidade (vs avulso)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **cliente com plano de fidelidade (vs avulso)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse cliente com plano de fidelidade (vs avulso) **se comporta diferente**: acumula pontos e ganha descontos."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **cliente com plano de fidelidade (vs avulso)** faça o seu (acumula pontos e ganha descontos) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de pagamento (dinheiro, cartão, pacote) sem refazer o sistema."*
...
- 🎯 **Direcionamento:** criem a interface `MeioDePagamento` para a forma de pagamento (dinheiro, cartão, pacote) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece agendar um serviço com um profissional."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "agendar um serviço com um profissional" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📨 **Cliente:** *"O agendamento passa por **fases** e **não pode pular etapa**."*
- 💡 **Exercita:** estados, transições, **guardas** (regra de negócio na transição).
- 📦 **Entregável:** **máquina de estados** de o agendamento: `marcado → em atendimento → concluído; ou cancelado/faltou`.
...
- 📨 **Cliente:** *"Escreve o **processo da marcação ao atendimento**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da marcação ao atendimento com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Agendamento cobra, calcula a comissão E manda o lembrete — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Agendamento cobra, calcula a comissão E manda o lembrete — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: o cálculo de comissão do profissional cheio de casos — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem o cálculo de comissão do profissional cheio de casos e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de agenda e financeiro (comissões). Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (agenda e financeiro (comissões)) e reorganizem o código em `com.salao.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em um computador na recepção e um app para a cliente agendar."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (um computador na recepção e um app para a cliente agendar) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: agendar um serviço com um profissional, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "agendar um serviço com um profissional" do início ao fim e apliquem **um** padrão onde couber (não force).
