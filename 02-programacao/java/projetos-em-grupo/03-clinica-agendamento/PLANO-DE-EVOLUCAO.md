...
> (dono de uma clínica) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar os pacientes**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista pacientes em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar as consultas de cada médico, e um médico atende várias por dia."*
...
- 📨 **Cliente (falando demais):** *"o paciente torce pro tal time, faz aniversário em maio, indicou um amigo…"* — 90% irrelevante.
...
- 📨 **Cliente:** *"Quero que cada consulta seja uma 'coisa' de verdade no sistema, não uma linha solta."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode marcar dois pacientes no mesmo horário do mesmo médico."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode marcar dois pacientes no mesmo horário do mesmo médico" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: marcaram uma consulta para as 25h. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode marcar dois pacientes no mesmo horário do mesmo médico": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: um médico tem muitas consultas; cada consulta tem um paciente e um médico — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (um médico tem muitas consultas; cada consulta tem um paciente e um médico) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: paciente de convênio (vs particular)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **paciente de convênio (vs particular)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse paciente de convênio (vs particular) **se comporta diferente**: tem valor diferente e exige autorização do convênio."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **paciente de convênio (vs particular)** faça o seu (tem valor diferente e exige autorização do convênio) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de cobrança (particular ou convênio) sem refazer o sistema."*
- 💡 **Exercita:** classe abstrata + **interface** = contrato plugável (baixo acoplamento).
- 📦 **Entregável:** uma **interface** `FormaDeCobranca` com **2 implementações** intercambiáveis.
- 🎯 **Direcionamento:** criem a interface `FormaDeCobranca` para a forma de cobrança (particular ou convênio) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece marcar uma consulta."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "marcar uma consulta" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📨 **Cliente:** *"A consulta passa por **fases** e **não pode pular etapa**."*
- 💡 **Exercita:** estados, transições, **guardas** (regra de negócio na transição).
- 📦 **Entregável:** **máquina de estados** de a consulta: `marcada → confirmada → realizada; ou cancelada/faltou`.
...
- 📨 **Cliente:** *"Escreve o **processo do agendamento até o atendimento**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem do agendamento até o atendimento com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Consulta agenda, cobra E emite o comprovante — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Consulta agenda, cobra E emite o comprovante — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: a verificação de horário livre repetida em vários lugares — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem a verificação de horário livre repetida em vários lugares e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de cobrança e prontuário. Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (cobrança e prontuário) e reorganizem o código em `com.clinica.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em computadores na recepção e nos consultórios, com um banco de dados central."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (computadores na recepção e nos consultórios, com um banco de dados central) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: marcar uma consulta, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "marcar uma consulta" do início ao fim e apliquem **um** padrão onde couber (não force).
