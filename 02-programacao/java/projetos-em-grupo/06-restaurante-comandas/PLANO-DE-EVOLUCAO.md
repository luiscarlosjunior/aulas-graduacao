...
> (dono de um restaurante) manda um **novo feedback**, e esse pedido só se resolve bem com o **conceito
...
- 📨 **Cliente:** *"Por enquanto, só quero **cadastrar e listar as mesas**. Nada de mais."*
- 💡 **Exercita:** variáveis, listas, laços, entrada/saída — **ainda sem classes**, de propósito.
- 📦 **Entregável:** um programa Java que cadastra e lista comandas em memória (tudo no `main`).
...
- 📨 **Cliente:** *"Ah, também preciso registrar os pedidos de cada comanda, e uma comanda acumula vários pedidos."*
...
- 📨 **Cliente (falando demais):** *"o comanda torce pro tal time, faz aniversário em maio, indicou um amigo…"* — 90% irrelevante.
...
- 📨 **Cliente:** *"Quero que cada comanda seja uma 'coisa' de verdade no sistema, não uma linha solta."*
...
- 📨 **Cliente:** *"Tem uma regra importante: não pode abrir duas comandas para a mesma mesa ao mesmo tempo."*
...
- 🎯 **Direcionamento:** escrevam a **assinatura** de cada operação (parâmetros e retorno) e a visibilidade de cada atributo. Transformem a regra "não pode abrir duas comandas para a mesma mesa ao mesmo tempo" numa operação.
...
- 📨 **Cliente:** *"Deu problema! Alguém colocou um valor absurdo: o preço de um item ficou negativo. Isso não podia acontecer."*
...
- 🎯 **Direcionamento:** implementem no código a regra "não pode abrir duas comandas para a mesma mesa ao mesmo tempo": o método valida **antes** de mudar o estado e recusa o inválido.
...
- 📨 **Cliente:** *"Preciso enxergar como as coisas se ligam: uma mesa tem uma comanda; a comanda é feita de vários pedidos — e quantas de cada lado."*
...
- 🎯 **Direcionamento:** desenhem a multiplicidade nas **duas pontas** (uma mesa tem uma comanda; a comanda é feita de vários pedidos) e, para cada "tem-um", decidam **composição (◆)** ou **agregação (◇)**.
...
- 📨 **Cliente:** *"Surgiu um **tipo especial**: cliente do rodízio (vs à la carte)."*
- 💡 **Exercita:** `extends`, `super`, reúso do que já existe.
- 📦 **Entregável:** uma **subclasse** que especializa a base — escrevendo **só o que muda**.
- 🎯 **Direcionamento:** criem a superclasse comum e façam **cliente do rodízio (vs à la carte)** estendê-la, reaproveitando o que já existe com `super`.
...
- 📨 **Cliente:** *"Esse cliente do rodízio (vs à la carte) **se comporta diferente**: paga um valor fixo em vez de somar item a item."*
...
- 🎯 **Direcionamento:** sobrescrevam um método para que **cliente do rodízio (vs à la carte)** faça o seu (paga um valor fixo em vez de somar item a item) e chamem sempre pela **referência do tipo base**.
...
- 📨 **Cliente:** *"Quero poder **trocar** a forma de dividir/pagar a conta (individual, por mesa) sem refazer o sistema."*
- 💡 **Exercita:** classe abstrata + **interface** = contrato plugável (baixo acoplamento).
- 📦 **Entregável:** uma **interface** `FormaDePagamento` com **2 implementações** intercambiáveis.
- 🎯 **Direcionamento:** criem a interface `FormaDePagamento` para a forma de dividir/pagar a conta (individual, por mesa) e façam **≥2 implementações**; o resto do sistema usa **só a interface**.
...
- 📨 **Cliente:** *"Me explica, **passo a passo**, como acontece abrir a comanda, pedir e fechar a conta."*
...
- 🎯 **Direcionamento:** desenhem a sequência de "abrir a comanda, pedir e fechar a conta" mensagem a mensagem, nomeando **quem chama quem**.
...
- 📨 **Cliente:** *"A comanda passa por **fases** e **não pode pular etapa**."*
- 💡 **Exercita:** estados, transições, **guardas** (regra de negócio na transição).
- 📦 **Entregável:** **máquina de estados** de a comanda: `aberta → fechada → paga; itens podem ser cancelados`.
...
- 📨 **Cliente:** *"Escreve o **processo da abertura da comanda ao pagamento**, quero treinar minha equipe com isso."*
...
- 🎯 **Direcionamento:** modelem da abertura da comanda ao pagamento com início, ações, decisões e fim; usem **raias** para mostrar o responsável de cada passo.
...
- 📦 **Entregável:** uma **refatoração** separando responsabilidades — por ex.: a classe Comanda soma a conta, aplica a taxa de serviço E imprime — separe.
- 🎯 **Direcionamento:** achem a classe que "faz demais" e quebrem em classes menores — comecem por: a classe Comanda soma a conta, aplica a taxa de serviço E imprime — separe.
...
- 📦 **Entregável:** uma **simplificação** — ex.: o cálculo de conta com taxa/serviço/desconto embolado — removendo duplicação e complexidade sem perder função.
- 🎯 **Direcionamento:** extraiam método para tirar código repetido, simplifiquem o cálculo de conta com taxa/serviço/desconto embolado e **apaguem** o que nenhum requisito pede.
...
- 📨 **Cliente:** *"Talvez **outro time cuide** da parte de salão (comandas) e cozinha (pedidos). Dá pra separar em módulos?"*
...
- 🎯 **Direcionamento:** agrupem as classes em pacotes por responsabilidade (salão (comandas) e cozinha (pedidos)) e reorganizem o código em `com.restaurante.{dominio,aplicacao,infra}`.
...
- 📨 **Cliente:** *"**Onde** isso vai rodar? Pensei em tablets dos garçons, uma tela na cozinha e um caixa."*
...
- 🎯 **Direcionamento:** desenhem onde cada parte roda (tablets dos garçons, uma tela na cozinha e um caixa) e rotulem o **protocolo** de cada conexão.
...
- 📨 **Cliente:** *"Quero ver **rodando de ponta a ponta**: abrir a comanda, pedir e fechar a conta, do jeito que a gente combinou."*
...
- 🎯 **Direcionamento:** montem um `Principal` que executa "abrir a comanda, pedir e fechar a conta" do início ao fim e apliquem **um** padrão onde couber (não force).
